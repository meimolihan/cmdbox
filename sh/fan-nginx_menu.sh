...
# 默认备份目录（保留 6 份；可在 77 处直接传参覆盖）
BACKUP_DIR="/vol2/1000/file/backup/fan-nginx-backup"
...
show_service_url() {
    ...
    # 优先从 systemd ExecStart 解析 --server.port
    if [ -z "$port" ];then
        local exec_cmd
        exec_cmd=$(systemctl show -p ExecStart "$service" 2>/dev/null | cut -d= -f2-)
        port=$(echo "$exec_cmd" | grep -oE '\-\-server\.port=[0-9]+' | grep -oE '[0-9]+' | head -1)
    fi

    # 回退：从启动日志（log file / journalctl）解析 Undertow 监听端口
    if [ -z "$port" ] && [ -f "/var/lib/fan-nginx/log/fan-nginx.log" ]; then
        port=$(grep -oE 'Undertow started on port\(s\) [0-9]+' /var/lib/fan-nginx/log/fan-nginx.log | grep -oE '[0-9]+$' | head -1)
    fi
    if [ -z "$port" ] && command -v journalctl >/dev/null 2>&1; then
        port=$(journalctl -u "$service" --no-pager -n 200 -o cat 2>/dev/null \
            | grep -oE 'Undertow started on port\(s\) [0-9]+' | grep -oE '[0-9]+$' | head -1)
    fi
...
manage_fan_nginx() {
    while true; do
        ...
        echo -e "${gl_bufan}9.  ${gl_bai}实时跟踪日志         ${gl_lv}10. ${gl_bai}查看/重置管理密码"
        echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
        ...
        10)
            echo -e ""
            echo -e "${gl_zi}>>> fan-nginx CLI：查看/重置管理密码 (credentials)${gl_huang}.${gl_huang}.${gl_lv}.${gl_bai}"
            echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
            echo -e "${gl_huang}注意：该操作会重置并打印全部管理员账号密码，并关闭两步验证。${gl_bai}"
            if command -v fan-nginx >/dev/null 2>&1; then
                sudo fan-nginx credentials
            else
                log_error "未找到 fan-nginx 命令，请先通过 66 安装/升级 fan-nginx"
            fi
            echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
            break_end
            ;;
        66)
            ...
            ;;
        77)
            bash <(curl -sL ${BACKUP_SCRIPT_URL}) "${BACKUP_DIR}" 6
            break_end
            continue
            ;;
        88)
            bash <(curl -sL ${RECOVER_SCRIPT_URL}) "${BACKUP_DIR}"
            break_end
            continue
            ;;
        99)
