...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="fan-nginx Nginx 图形化管理平台 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/fan-nginx"

# 默认访问端口（不传参时使用；--net=host 直通本机，无端口映射）
DEFAULT_PORT="8080"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="fan-nginx"

# 宿主/容器数据目录（bind 挂载，升级镜像不丢数据）
DEFAULT_HOST_DATA_DIR="/home/fan-nginx"

# 初始管理员账号（写入 BOOT_OPTIONS --init.admin，仅首次启动生效）
DEFAULT_INIT_ADMIN="admin"

# 初始管理员密码（写入 BOOT_OPTIONS --init.pass，仅首次启动生效）
DEFAULT_INIT_PASS="admin"
# ====================================================================================
...
deploy_app() {
    ...
    if [ -z "${HOST_PORT}" ]; then
        read -r -e -p "${gl_bai}请输入访问端口（回车默认：${gl_huang}${DEFAULT_PORT}${gl_bai}）(${gl_hong}0${gl_bai} 退出安装)：" input_port
    ...
    # 初始管理员账号/密码（可提前用环境变量 INIT_ADMIN / INIT_PASS 指定）
    INIT_ADMIN="${INIT_ADMIN:-$DEFAULT_INIT_ADMIN}"
    INIT_PASS="${INIT_PASS:-$DEFAULT_INIT_PASS}"
    if [ -t 0 ]; then
        read -r -e -p "${gl_bai}请输入初始管理员账号（回车默认：${gl_huang}${INIT_ADMIN}${gl_bai}）：" input_admin
        INIT_ADMIN=${input_admin:-$INIT_ADMIN}
        read -r -e -p "${gl_bai}请输入初始管理员密码（回车默认：${gl_huang}${INIT_PASS}${gl_bai}）(${gl_hong}建议修改${gl_bai})：" input_pass
        INIT_PASS=${input_pass:-$INIT_PASS}
    else
        log_info "非交互模式，使用默认初始账号/密码：${gl_huang}${INIT_ADMIN}${gl_bai} / ${gl_huang}${INIT_PASS}${gl_bai}"
    fi
...
services:
   fan-nginx:
      ...
      container_name: fan-nginx
      network_mode: "host"
      restart: always
      volumes:
         - type: bind
           source: "${DEFAULT_HOST_DATA_DIR}"
           target: "${DEFAULT_HOST_DATA_DIR}"
      environment:
         BOOT_OPTIONS: "--server.port=${HOST_PORT} --init.admin=${INIT_ADMIN} --init.pass=${INIT_PASS}"
EOF
    ...
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    log_info "数据目录：${gl_huang}${DEFAULT_HOST_DATA_DIR}${gl_bai}（bind 挂载，升级镜像不丢数据）"
    log_info "日志位置：${gl_huang}${DEFAULT_HOST_DATA_DIR}/log/fan-nginx.log${gl_bai}"
    log_info "首次登录：账号 ${gl_lv}${INIT_ADMIN}${gl_bai}，密码 ${gl_huang}${INIT_PASS}${gl_bai}（仅首次启动生效，登录后请立即修改）"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
