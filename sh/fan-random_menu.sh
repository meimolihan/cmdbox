...
list_color_init
SERVICE="fan-random"
CONFIG_FILE="/etc/fan-random.conf"
...
show_service_url() {
    ...
    # 1) 安装记录 /etc/fan-random.conf 中读取端口
    if [ -f "${CONFIG_FILE}" ]; then
        port=$(sed -n 's/^PORT=//p' "${CONFIG_FILE}" | tail -n1 | tr -d '[:space:]')
    fi

    # 2) systemd 单元 Environment 中读取 PORT
    if [ -z "$port" ]; then
    ...
    # 4) 使用内置 CLI 状态输出兜底
    if [ -z "$port" ] && command -v fan-random >/dev/null 2>&1; then
        port=$(fan-random status 2>/dev/null | grep -oE '监听端口[^0-9]*[0-9]+' | grep -oE '[0-9]+$' | head -1)
    fi
    ...
    if [ -n "$url" ]; then
        echo -e "访问地址：${gl_lv}${url}${gl_bai}  （ / pc 桌面端  ${url}/mp 移动端 ）"
    else
