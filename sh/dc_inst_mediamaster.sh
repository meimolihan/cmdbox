...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="MediaMaster 媒资管理 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/mediamaster"

# 默认媒体库目录
DEFAULT_MEDIA_DIR="/vol2/1000/media"

# 默认下载目录
DEFAULT_DOWNLOADS_DIR="/vol2/1000/downloads/mediamaster"

# 默认 Web 访问端口（不传参时使用），映射容器 8888
DEFAULT_PORT="1688"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="mediamaster"
# ====================================================================================
...
deploy_app() {
    ...
    if [ -z "${MEDIA_DIR}" ]; then
        read -r -e -p "${gl_bai}请输入媒体库目录（回车默认：${gl_huang}${DEFAULT_MEDIA_DIR}${gl_bai}）：" input_media
    ...
    if [ -z "${DOWNLOADS_DIR}" ]; then
        read -r -e -p "${gl_bai}请输入下载目录（回车默认：${gl_huang}${DEFAULT_DOWNLOADS_DIR}${gl_bai}）：" input_downloads
        DOWNLOADS_DIR=${input_downloads:-$DEFAULT_DOWNLOADS_DIR}
    fi

    if [ ! -d "$DOWNLOADS_DIR" ]; then
        log_warn "目录 ${gl_huang}${DOWNLOADS_DIR}${gl_bai} 不存在，将自动创建"
