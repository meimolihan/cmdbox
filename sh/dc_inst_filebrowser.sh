...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="FileBrowser 网页文件管理器 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/filebrowser"

# 默认访问端口（不传参时使用），映射容器 80
DEFAULT_PORT="8880"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="filebrowser"
# ====================================================================================
...
deploy_app() {
    ...
    mkdir -p "${COMPOSE_DIR}/config" "${COMPOSE_DIR}/db" || { log_error "目录创建失败"; break_end; return 1; }
...
EOF
    ...
    log_info "等待初始化并获取默认密码，请稍候 ${gl_hong}.${gl_huang}.${gl_lv}.${gl_bai}"
    sleep 8
    FB_PASSWORD=$(docker logs "${DEFAULT_CONTAINER_NAME}" 2>&1 | sed -n 's/.*randomly generated password: //p' | head -n1)
    if [[ -z "${FB_PASSWORD}" ]]; then
        FB_PASSWORD="获取失败，执行命令查看：docker logs ${DEFAULT_CONTAINER_NAME} | grep password"
    fi
    ...
    log_info "登录账号：${gl_lv}admin${gl_bai}"
    log_info "登录密码：${gl_huang}${FB_PASSWORD}${gl_bai}"
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
