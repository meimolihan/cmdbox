...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="Arcane 应用管理器 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/arcane"

# 默认访问端口（不传参时使用）
DEFAULT_PORT="3552"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="arcane"
# ====================================================================================
...
generate_key() {
    ...
    if [ $? -ne 0 ]; then
        echo "Arcane$(date +%s)$(hostname | md5sum | head -c 16)"
    fi
}
...
deploy_app() {
    ...
    local ENCRYPTION_KEY=$(generate_key)
