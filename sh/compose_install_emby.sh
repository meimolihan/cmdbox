...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="Emby 媒体服务器 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/emby"

# 默认 Web 访问端口（不传参时使用），映射容器 8096
DEFAULT_PORT="8996"
...
deploy_app() {
    ...
    mkdir -p "${COMPOSE_DIR}/video" || { log_error "视频目录创建失败"; break_end; return 1; }
    ...
    log_info "使用 Web 端口：${gl_lv}${HOST_PORT}${gl_bai}（HTTPS 端口固定：8920）"
...
services:
   ${DEFAULT_CONTAINER_NAME}:
      ...
      extra_hosts:
         ...
         - themoviedb.org:99.84.238.136
         - api4.thetvdb.com:13.225.142.99
EOF
    ...
    log_info "HTTPS 访问地址：${gl_lv}https://${LOCAL_IP}:8920${gl_bai}"
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    log_info "视频目录：${gl_huang}${COMPOSE_DIR}/video${gl_bai}"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
