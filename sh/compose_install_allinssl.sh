...
# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/allinssl"

# 默认访问端口（不传参时使用）
DEFAULT_PORT="7979"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="allinssl"
# ====================================================================================
...
services:
   ${DEFAULT_CONTAINER_NAME}:
      ...
      volumes:
         ...
         - /etc/nginx/keyfile:/www/allinssl/keyfile # 映射 nginx 证书目录
      environment:
         - ALLINSSL_USER=admin                      # 修改为管理用户名
         - ALLINSSL_PWD=12345678                    # 修改为管理密码
         - ALLINSSL_URL=allinssl                    # 修改为访问路径
EOF
    ...
    log_info "部署完成！"
    log_info "访问地址：${gl_lv}http://${LOCAL_IP}:${HOST_PORT}/allinssl${gl_bai}"
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
