...
# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="speedtest"
# ====================================================================================
...
deploy_app() {
    ...
    mkdir -p "${COMPOSE_DIR}/config" || { log_error "目录创建失败"; break_end; return 1; }
...
EOF
    ...
    echo -e ""
    echo -e "${gl_huang}>>> 等待服务初始化就绪 ${gl_hong}.${gl_huang}.${gl_lv}.${gl_bai}"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    local max_retry=30
    local ready=0
    for i in $(seq 1 $max_retry); do
        if curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:${HOST_PORT} 2>/dev/null | grep -q "200\|302"; then
            ready=1
            break
        fi
        sleep 2
    ...
    if [ "$ready" -eq 1 ]; then
        log_ok "Speedtest Go 服务就绪"
    else
        log_warn "等待超时，服务可能仍在初始化，请稍后手动刷新"
