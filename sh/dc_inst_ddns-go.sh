...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="DDNS-GO 动态域名解析 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/ddns-go"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="ddns-go"
# ====================================================================================
...
EOF
    ...
    log_info "部署完成！"
    log_info "访问地址：${gl_lv}http://${LOCAL_IP}:9876${gl_bai}"
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    echo -e ""
    log_info "首次使用说明："
    log_info "1. 打开 Web UI：${gl_lv}http://${LOCAL_IP}:9876${gl_bai}"
    log_info "2. 选择 DNS 服务商（阿里云 / 腾讯云 / Cloudflare 等）"
    log_info "3. 填写 API 凭证和需要解析的域名"
    log_info "4. 点击保存，自动开始动态域名解析"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
