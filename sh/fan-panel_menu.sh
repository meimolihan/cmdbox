...
SERVICE="fan-panel"
RECORD_FILE="/etc/fan-panel.conf"
...
show_service_url() {
    ...
    port=$(journalctl -u "$service" --no-pager -n 200 -o cat 2>/dev/null \
        | grep -E 'Listening and serving HTTP on :[0-9]+' \
        | grep -oE ':[0-9]+$' | sed 's/^://' | head -1)

    if [ -z "$port" ];then
        local app_dir=""
        [ -f "${RECORD_FILE}" ] && app_dir=$(grep -E '^APP_DIR=' "${RECORD_FILE}" | cut -d= -f2)
        app_dir="${app_dir:-/var/lib/fan-panel}"
        port=$(grep -E '^http_port=' "${app_dir}/conf/conf.ini" 2>/dev/null | cut -d= -f2 | tr -d ' \r')
    fi
...
show_service_status() {
    ...
    if [ -f "${RECORD_FILE}" ]; then
        version=$(grep -E '^VERSION=' "${RECORD_FILE}" 2>/dev/null | cut -d= -f2 | tr -d ' \r')
    fi

    if [ -z "$version" ] && command -v journalctl &>/dev/null; then
        version=$(journalctl -u "$service" --no-pager -n 50 -o cat 2>/dev/null | grep -oE '\bv[0-9]+\.[0-9]+\.[0-9]+\b' | head -1)
    fi
