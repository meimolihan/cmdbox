...
show_service_url() {
    ...
    port=$(journalctl -u "$service" --no-pager -n 200 -o cat 2>/dev/null \
        | grep -E 'Listening on \[::\]:[0-9]+' \
        | grep -oE '[0-9]+$' | head -1)

    if [ -z "$port" ];then
        local env_line
        env_line=$(systemctl show -p Environment "$service" 2>/dev/null | cut -d= -f2-)
        port=$(echo "$env_line" | tr ' ' '\n' | grep -oE '^PORT=[0-9]+' | cut -d= -f2 | head -1)
    fi
...
manage_fan_reubah() {
    while true; do
        ...
        echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
        echo -e "${gl_lv}66. ${gl_bai}安装/升级 fan-reubah"
        echo -e "${gl_hong}99. ${gl_bai}卸载 fan-reubah"
