...
# 备份/恢复默认目录（可改）
BACKUP_DIR="/var/lib/fan-webssh/backup"
KEEP_NUM=6
...
show_service_url() {
    ...
    port=$(journalctl -u "$service" --no-pager -n 200 -o cat 2>/dev/null \
        | grep -E 'Server\(http\) is running on: http://localhost:[0-9]+' \
...
manage_fan_webssh() {
    while true; do
        ...
        66)
            ...
            ;;
        77)
            bash <(curl -sL ${BACKUP_SCRIPT_URL}) "${BACKUP_DIR}" ${KEEP_NUM}
            break_end
            continue
            ;;
        88)
