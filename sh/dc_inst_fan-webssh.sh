...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="fan-webssh 多功能Linux终端面板(webSSH&webSFTP) 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/fan-webssh"

# 默认访问端口（不传参时使用）
DEFAULT_PORT="8082"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="fan-webssh"

# RDP(guacd) 依赖容器名称
DEFAULT_GUACD_CONTAINER_NAME="guacd"

# 数据目录挂载（只读挂载可去掉 :ro；数据库持久化到宿主机 ./db）
DEFAULT_DB_DIR="./db"
# ====================================================================================
...
deploy_app() {
    ...
    clean_old_container "${DEFAULT_CONTAINER_NAME}" "${DEFAULT_GUACD_CONTAINER_NAME}"
...
services:
   fan-webssh:
      image: mobufan/fan-webssh:latest
      container_name: fan-webssh
      network_mode: bridge
      restart: always
      environment:
         - TZ=Asia/Shanghai
         ...
         - GUACD_PORT=4822
      volumes:
         # 数据库持久化（含 key.db/group.db/host.db 等全部面板数据）
         - ./db:/fan-webssh/app/db
         # 面板 Docker 容器管理功能需要 docker.sock（如不需要可注释本行）
...
EOF
    ...
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    log_info "数据目录：${gl_huang}${COMPOSE_DIR}/db${gl_bai} (全部面板数据库，持久化挂载)"
    log_info "首次登录：账号/密码为随机生成，见容器日志：${gl_huang}docker logs fan-webssh 2>&1 | grep -iE '账号|密码|password'${gl_bai}（登录后建议立即修改）"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
