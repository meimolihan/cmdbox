...
show_service_url() {
    ...
    port=$(journalctl -u "$service" --no-pager -n 200 -o cat 2>/dev/null \
        | grep -E 'Uvicorn running on http://0\.0\.0\.0:[0-9]+' \
        | grep -oE '0\.0\.0\.0:[0-9]+' | grep -oE '[0-9]+$' | head -1)

    if [ -z "$port" ];then
        local exec_line
        exec_line=$(systemctl show -p ExecStart "$service" 2>/dev/null | cut -d= -f2-)
        port=$(echo "$exec_line" | grep -oE -- '--port [0-9]+' | grep -oE '[0-9]+$' | head -1)
    fi
...
show_service_status() {
    ...
    if [ -z "$version" ] && [ -f /opt/fan-shop/backend/app/version.py ]; then
        version=$(grep -E '^VERSION' /opt/fan-shop/backend/app/version.py 2>/dev/null | sed -n 's/.*"\(v[^"]*\)".*/\1/p' | head -1)
    fi
