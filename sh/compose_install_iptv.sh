...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="IPTV 直播流代理服务 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/iptv"

# 默认访问端口（不传参时使用）
DEFAULT_PORT="1905"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="iptv"
# ====================================================================================
...
deploy_app() {
    ...
    echo ""
    log_info "请输入 muserId（留空为游客模式）："
    read -r -e -p "${gl_bai}> ${gl_bai}" INPUT_USER_ID

    echo ""
    log_info "请输入 mtoken（留空为游客模式，用于高画质/VIP）："
    read -r -e -p "${gl_bai}> ${gl_bai}" INPUT_TOKEN
...
services:
   ${DEFAULT_CONTAINER_NAME}:
      ...
      ports:
         - ${HOST_PORT}:1905
      environment:
         - muserId=${INPUT_USER_ID}  # 可选：咪咕账号ID（留空为游客模式）
         - mtoken=${INPUT_TOKEN}    # 可选：咪咕登录令牌（用于高画质/VIP）
         - mport=${HOST_PORT}
