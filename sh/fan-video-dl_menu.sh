...
show_service_url() {
    ...
    # 1) 优先读取安装记录配置文件（install.sh 写入 PORT=xxxx）
    port=$(grep -E '^PORT=' /etc/fan-video-dl.conf 2>/dev/null | head -1 | cut -d= -f2)

    # 2) 从 gunicorn 日志提取监听端口
    if [ -z "$port" ]; then
        port=$(journalctl -u "$service" --no-pager -n 200 -o cat 2>/dev/null \
            | grep -E 'Listening at: http://0\.0\.0\.0:[0-9]+' \
            | grep -oE ':[0-9]+$' | sed 's/^://' | head -1)
    fi

    # 3) 从 ExecStart 提取 --bind 0.0.0.0:PORT
    if [ -z "$port" ];then
        local exec_cmd
        exec_cmd=$(systemctl show -p ExecStart "$service" 2>/dev/null | cut -d= -f2-)
        port=$(echo "$exec_cmd" | grep -oE '0\.0\.0\.0:[0-9]+' | grep -oE '[0-9]+' | head -1)
    fi
