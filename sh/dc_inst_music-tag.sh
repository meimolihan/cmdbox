...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="Music Tag Web 音乐标签编辑器 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/music-tag"

# 默认访问端口（不传参时使用）
DEFAULT_PORT="8001"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="music-tag"
# 内置容器端口固定8002，无需修改
INNER_PORT="8002"
# ====================================================================================
...
clean_old_container() {
    ...
    log_info "清空旧配置数据（重置登录账号） ${gl_hong}.${gl_huang}.${gl_lv}.${gl_bai}"
    rm -rf ./config ./music
    log_ok "旧账号数据已清空"
...
EOF
    ...
    # 自动全局权限修复
    echo -e ""
    echo -e "${gl_huang}>>> 自动修复目录读写权限${gl_bai}"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    chmod -R 777 "${COMPOSE_DIR}"
    ...
    # 等待服务初始化
    log_info "等待服务初始化完成，请稍等10秒 ${gl_hong}.${gl_huang}.${gl_lv}.${gl_bai}"
    sleep 10
    ...
    log_info "🎉 部署完成！"
    log_info "前台访问地址：${gl_lv}http://${LOCAL_IP}:${HOST_PORT}${gl_bai}"
    log_info "后台管理地址：${gl_lv}http://${LOCAL_IP}:${HOST_PORT}/admin${gl_bai}"
    log_info "✅ 登录用户名：${gl_lv}admin${gl_bai}"
    log_info "✅ 登录密码：${gl_lv}admin${gl_bai}"
    log_info "本地音乐存放目录：${gl_huang}${COMPOSE_DIR}/music${gl_bai}"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
