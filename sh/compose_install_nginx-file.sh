...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="Nginx 轻量级文件服务 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/nginx-file"

# 默认访问端口（不传参时使用）
DEFAULT_PORT="18080"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="nginx-file"
# ====================================================================================
...
# 自动关闭SELinux + 修复目录权限
fix_selinux_and_perm() {
    log_info "检测并临时关闭 SELinux（解决目录访问403）"
    if command -v getenforce &>/dev/null; then
        local sel_status=$(getenforce)
        if [ "${sel_status}" = "Enforcing" ]; then
            setenforce 0
            log_warn "SELinux 已临时关闭，重启服务器后需重新执行"
        else
            log_ok "SELinux 已处于关闭状态"
        fi
    else
        log_info "当前系统未启用 SELinux，无需处理"
    fi

    log_info "修正文件目录权限与归属，防止403"
    mkdir -p ./data
    chmod -R 755 ./data
    ...
    if [ ! -f ./data/index.html ]; then
        echo "<h3>Nginx 文件服务</h3><p>文件列表浏览正常</p>" > ./data/index.html
        log_ok "自动创建 index.html 首页文件"
    fi
}
...
deploy_app() {
    ...
    # 关键修复：如果 nginx.conf 是目录，直接删除
    if [ -d "nginx.conf" ]; then
        log_warn "检测到 nginx.conf 为目录，正在删除"
        rm -rf nginx.conf
    fi
...
EOF
    ...
    echo -e ""
    echo -e "${gl_huang}>>> 生成 ${gl_lv}nginx.conf${gl_huang} 配置文件 ${gl_hong}.${gl_huang}.${gl_lv}.${gl_bai}"
    cat > nginx.conf << EOF
user  nginx;
...
EOF
    log_ok "nginx.conf 已生成（优化目录浏览权限）"
    ...
    log_info "文件目录：${gl_huang}${COMPOSE_DIR}/data${gl_bai}（放入文件即可访问）"
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
