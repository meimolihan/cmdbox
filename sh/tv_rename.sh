...
if ! TARGET_DIR=$(realpath -- "$TARGET_DIR" 2>/dev/null); then
    echo -e "${gl_hong}[错误]${gl_bai} 路径解析失败，请检查路径格式${gl_end}"
    exit 1
fi

if [[ ! -d "$TARGET_DIR" ]]; then
    echo -e "${gl_hong}[错误]${gl_bai} 目录不存在：$TARGET_DIR${gl_end}"
    exit 1
fi

if ! cd -- "$TARGET_DIR" 2>/dev/null; then
    echo -e "${gl_hong}[错误]${gl_bai} 无法进入目录，权限不足：$TARGET_DIR${gl_end}"
    exit 1
fi

log_info() { echo -e "${gl_lan}[信息]${gl_bai} $*${gl_end}"; }
log_ok() { echo -e "${gl_lv}[成功]${gl_bai} $*${gl_end}"; }
log_warn() { echo -e "${gl_huang}[警告]${gl_bai} $*${gl_end}"; }
log_error() { echo -e "${gl_hong}[错误]${gl_bai} $*${gl_end}" >&2; }

break_end() {
    echo -e "${gl_lv}操作完成${gl_bai}${gl_end}"
    echo -e "${gl_bai}按任意键继续 ${gl_hong}.${gl_huang}.${gl_lv}.${gl_bai}\c${gl_end}"
    read -r -n 1 -s -p ""
    echo ""
    clear
}
...
cancel_return() {
    ...
    echo -ne "${gl_lv}即将返回 ${gl_huang}${menu_name} ${gl_hong}.${gl_huang}.${gl_lv}.${gl_bai}\c${gl_end}"
    sleep_fractional 0.5
    echo -ne "${gl_hong}.${gl_huang}.${gl_lv}.${gl_bai}\c${gl_end}"
...
handle_y_n() {
    echo -ne "\r${gl_hong}无效的选择，请输入 ${gl_bai}(${gl_lv}y${gl_bai}或${gl_hong}N${gl_bai}) ${gl_hong}.${gl_huang}.${gl_lv}.${gl_bai}\c${gl_end}"
    sleep_fractional 0.3
    echo -ne "\r${gl_huang}无效的选择，请输入 ${gl_bai}(${gl_lv}y${gl_bai}或${gl_hong}N${gl_bai}) ${gl_hong}.${gl_huang}.${gl_lv}.${gl_bai}\c${gl_end}"
    sleep_fractional 0.3
    echo -ne "\r${gl_lv}无效的选择，请输入 ${gl_bai}(${gl_lv}y${gl_bai}或${gl_hong}N${gl_bai}) ${gl_hong}.${gl_huang}.${gl_lv}.${gl_bai}\c${gl_end}"
    sleep_fractional 0.6
    echo ""
    return 2
}
...
exit_animation() {
    echo -ne "\r${gl_lv}即将退出 ${gl_hong}.${gl_huang}.${gl_lv}.${gl_bai}\c${gl_end}"
    sleep_fractional 1
    echo -ne "${gl_hong}.${gl_huang}.${gl_lv}.${gl_bai}\c${gl_end}"
    sleep_fractional 1
    echo ""
...
exit_script() {
    echo ""
    echo -ne "${gl_hong}感谢使用，再见！${gl_hong}.${gl_huang}.${gl_lv}.${gl_bai}\c${gl_end}"
    sleep_fractional 0.5
    echo -ne "${gl_hong}.${gl_huang}.${gl_lv}.${gl_bai}\c${gl_end}"
    sleep_fractional 0.6
    clear
...
handle_invalid_input() {
    echo -ne "\r\033[K${gl_huang}无效的输入,请重新输入! ${gl_zi} 1 ${gl_huang} 秒后返回${gl_end}"
    sleep_fractional 1
    echo -ne "\r\033[K${gl_lv}无效的输入,请重新输入! ${gl_zi}0${gl_lv} 秒后返回${gl_end}"
    sleep_fractional 0.5
    echo -ne "\r\033[K"
    continue
}
...
rename_tv_files_ultimate() {
    ...
    analyze_filename_patterns() {
        local files=("$@")
        echo -e "${gl_zi}>>> 智能模式分析${gl_bai}${gl_end}"
        echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
        local pattern_array=()
        local example_array=()
        local count_array=()
        ...
        if [ ${#pattern_array[@]} -eq 0 ]; then
            echo -e "${gl_huang}未检测到有效的剧集编号模式${gl_bai}${gl_end}"
        else
            echo -e "${gl_bai}检测到的剧集编号模式:${gl_end}"
            echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
            for ((i = 0; i < ${#pattern_array[@]}; i++)); do
                ...
                if [ $percentage -ge 50 ]; then
                    echo -e "  ${gl_lv}${index}.${gl_bai} ${gl_bufan}${type}${gl_bai} (${percentage}% 文件)${gl_end}"
                else
                    echo -e "  ${gl_huang}${index}.${gl_bai} ${gl_bufan}${type}${gl_bai} (${percentage}% 文件)${gl_end}"
                fi
                echo -e "     示例: ${gl_hui}${example}${gl_bai}${gl_end}"
            done
        fi
        echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
        return 0
    ...
    auto_detect_rename_plan() {
        ...
        local plans=()
        clear
        echo -e "${gl_zi}>>> 自动检测重命名方案${gl_bai}${gl_end}"
        echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
        local season_array=()
        ...
        if [ ${#plans[@]} -eq 0 ]; then
            echo -e "${gl_huang}未检测到有效的重命名方案${gl_bai}${gl_end}"
            return 1
        fi
        echo -e "${gl_bai}检测到的重命名方案:${gl_end}"
        echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
        for ((i = 0; i < ${#plans[@]}; i++)); do
            ...
            *) color="${gl_hong}" ;;
            ...
            echo -e "  ${gl_bufan}${index}.${gl_bai} ${color}${type}${gl_bai}${gl_end}"
            echo -e "     值: ${gl_bufan}${value}${gl_bai}${gl_end}"
            echo -e "     说明: ${gl_hui}${description}${gl_bai}${gl_end}"
            echo -e "     置信度: ${color}${confidence}${gl_bai}${gl_end}"
            echo ""
        done
        echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
        return 0
    ...
    while true; do
        clear
        echo -e "${gl_zi}>>> 电视剧文件重命名（终极版）${gl_bai}${gl_end}"
        echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
        ...
        if [ ${#files[@]} -eq 0 ]; then
            ...
            echo -e "${gl_bai}支持格式: ${gl_bufan}${VIDEO_EXTS[*]}${gl_bai}${gl_end}"
            echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
            echo -e "${gl_huang}0.  ${gl_bai}返回上一级选单${gl_end}"
            echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
        ...
        echo -e "${gl_bufan}找到 ${#files[@]} 个可识别文件:${gl_bai}${gl_end}"
        for ((i = 0; i < ${#files[@]}; i++)); do
            local filename=$(basename -- "${files[$i]}")
            IFS=':' read -r episode season type <<<"${episode_info[$i]}"
            echo -e "  ${gl_bufan}$(safe_printf "%02d" $((i + 1))).${gl_bai} E$(safe_printf "%02d" "$episode") [${type}] - $filename${gl_end}"
        done
        if [ $auto_detected -eq 0 ] && [ ${#files[@]} -gt 0 ]; then
            echo -e ""
            echo -e "${gl_bai}检测到的模式分布:${gl_end}"
            local pattern_array=()
            local count_array=()
            ...
            for i in "${!pattern_array[@]}"; do
                ...
                local count="${count_array[i]}"
                local percentage=$((count * 100 / ${#files[@]}))
                echo -e "  ${gl_bufan}${pattern}:${gl_bai} ${count} 文件 (${percentage}%)${gl_end}"
            done
        fi

        echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
        if [ -n "$PREFIX" ]; then
            echo -e "${gl_bufan}当前设置:${gl_bai}${gl_end}"
            echo -e "  ${gl_bufan}剧集前缀:${gl_bai} $PREFIX${gl_end}"
            echo -e "  ${gl_bufan}起始季号:${gl_bai} S$SEASON${gl_end}"
            echo -e "  ${gl_bufan}起始集数:${gl_bai} E$(safe_printf "%02d" $START_EP)${gl_end}"
            if [ $auto_detected -eq 1 ]; then
                echo -e "  ${gl_lv}✓ 智能检测已应用${gl_bai}${gl_end}"
            fi
            echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
        fi

        echo -e "${gl_bufan}1.  ${gl_bai}设置剧集前缀 ${gl_hong}★ 必须设置${gl_end}"
        echo -e "${gl_bufan}2.  ${gl_bai}设置起始季号 ${gl_huang}★ 默认S01（非必选）${gl_end}"
        echo -e "${gl_bufan}3.  ${gl_bai}设置起始集数 ${gl_huang}★ 默认E01（非必选）${gl_end}"
        echo -e "${gl_bufan}4.  ${gl_bai}预览命名结果 ${gl_hong}★ 强制预览${gl_end}"
        if [ $preview_mode -eq 1 ] && [ -n "$PREFIX" ]; then
            echo -e "${gl_bufan}5.  ${gl_lv}执行重命名   ✓ 可执行${gl_end}"
        else
            echo -e "${gl_bufan}5.  ${gl_bai}执行重命名   ${gl_hong}✗ 不可执行${gl_end}"
        fi
        echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
        echo -e "${gl_hong}0.  ${gl_bai}退出脚本${gl_end}"
        echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
        read -r -e -p "$(echo -e "${gl_bai}请输入你的选择: ")" choice
        case $choice in
        1)
            clear
            echo -e "${gl_zi}>>> 设置剧集前缀${gl_bai}${gl_end}"
            echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
            ...
            echo -e "${gl_bai}从文件名中提取到的剧名前缀:${gl_end}"
            for i in "${!candidates[@]}"; do
                local idx=$((i + 1))
                local percentage=$((counts[i] * 100 / total_files))
                echo -e "  ${gl_bufan}${idx}.${gl_bai} ${gl_lv}${candidates[$i]}${gl_bai} (出现 ${counts[$i]} 次, ${percentage}%)${gl_end}"
            done
            echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
            echo -e "${gl_bai}推荐: ${gl_lv}${candidates[0]}${gl_bai} (${gl_huang}回车直接使用推荐${gl_bai})${gl_end}"
            echo -e "${gl_bai}示例: ${gl_bufan}${candidates[0]}-S${SEASON}E01.扩展名${gl_bai}${gl_end}"
            echo -e "${gl_bai}指定: ${gl_bai}手动输入自定义前缀${gl_end}"
            echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
        ...
        2)
            echo -e ""
            echo -e "${gl_zi}>>> 设置季号${gl_bai}${gl_end}"
            echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
            local season_array=()
            local season_count_array=()
            ...
            if [ ${#season_array[@]} -gt 0 ]; then
                echo -e "${gl_bai}从文件名检测到季号:${gl_end}"
                for i in "${!season_array[@]}"; do
                    local season="${season_array[i]}"
                    local count="${season_count_array[i]}"
                    local percentage=$((count * 100 / ${#files[@]}))
                    echo -e "  ${gl_bufan}S${season}:${gl_bai} ${count} 文件 (${percentage}%)${gl_end}"
                done
                echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
            fi
            echo -e "${gl_bai}当前季号: ${gl_bufan}S${SEASON}${gl_bai}${gl_end}"
            echo -e "${gl_bai}示例: ${gl_bufan}01${gl_bai} 表示第 ${gl_bufan}1${gl_bai} 季 (将显示为 S01)${gl_end}"
            echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
            ...
            if [ -z "$input_season" ]; then
                echo -e "${gl_bai}保持当前季号: S${SEASON}${gl_end}"
                continue
            fi
        ...
        3)
            echo -e ""
            echo -e "${gl_zi}>>> 设置起始集数${gl_bai}${gl_end}"
            echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
            if [ ${#episode_info[@]} -gt 0 ]; then
                ...
                echo -e "${gl_bai}识别的集数范围:${gl_end}"
                echo -e "  ${gl_bufan}最小:${gl_bai} E$(safe_printf "%02d" "$first_ep") (${first_type})${gl_end}"
                echo -e "  ${gl_bufan}最大:${gl_bai} E$(safe_printf "%02d" "$last_ep") (${last_type})${gl_end}"
                echo -e "  ${gl_bufan}建议:${gl_bai} 从 E$(safe_printf "%02d" "$first_ep") 开始${gl_end}"
                echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
            fi
            echo -e "${gl_bai}当前起始集数: ${gl_bufan}E$(safe_printf "%02d" "$START_EP")${gl_bai}${gl_end}"
            echo -e "${gl_bai}示例: ${gl_bufan}26${gl_bai} 表示从第 ${gl_bufan}26${gl_bai} 集开始编号${gl_end}"
            echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
            ...
            if [ -z "$input_start" ]; then
                echo -e "${gl_bai}保持当前起始集数: E$(safe_printf "%02d" "$START_EP")${gl_end}"
                continue
            fi
        ...
        4)
            ...
            clear
            echo -e "${gl_zi}>>> 预览重命名结果${gl_bai}${gl_end}"
            echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
            echo -e "${gl_bufan}当前设置:${gl_bai}${gl_end}"
            echo -e "  ${gl_bufan}剧集前缀:${gl_bai} $PREFIX${gl_end}"
            echo -e "  ${gl_bufan}起始季号:${gl_bai} S$SEASON${gl_end}"
            ...
            for ((i = 0; i < ${#files[@]}; i++)); do
                ...
                echo -e "  ${gl_bufan}[$(safe_printf "%02d" $((i + 1)))]${gl_bai}${gl_end}"
                echo -e "    原文件: ${gl_hui}$filename${gl_bai}${gl_end}"
                echo -e "    识别为: ${type} (E$(safe_printf "%02d" "$original_ep"))${gl_end}"
                echo -e "    新文件: ${gl_lv}$new_name${gl_bai}${gl_end}"
                echo ""
                detection_results+=("$file:$new_name:$type:$original_ep")
                ((current_ep++))
            done
            if [ ${#summary_array[@]} -gt 0 ]; then
                echo -e "${gl_bai}识别模式统计:${gl_end}"
                for i in "${!summary_array[@]}"; do
                    local type="${summary_array[i]}"
                    local count="${summary_count_array[i]}"
                    local percentage=$((count * 100 / ${#files[@]}))
                    echo -e "  ${gl_bufan}${type}:${gl_bai} ${count} 文件 (${percentage}%)${gl_end}"
                done
            fi
            preview_mode=1
            echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
        ...
        5)
            ...
            clear
            echo -e "${gl_zi}>>> 确认重命名${gl_bai}${gl_end}"
            echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
            local type_array=()
            ...
            echo -e "${gl_bai}重命名统计:${gl_end}"
            echo -e "  ${gl_bufan}总文件数:${gl_bai} ${#detection_results[@]}${gl_end}"
            for i in "${!type_array[@]}"; do
                local type="${type_array[i]}"
                local count="${type_count_array[i]}"
                echo -e "  ${gl_bufan}${type}:${gl_bai} ${count} 文件${gl_end}"
            done
            echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
            ...
            [Yy])
                ...
                for item in "${detection_results[@]}"; do
                    IFS=':' read -r old_file new_name type original_ep <<<"$item"
                    if [ -f "$old_file" ]; then
                        echo -e "${gl_bai}处理: ${gl_hui}$(basename "$old_file")${gl_bai}${gl_end}"
                        echo -e "  识别: ${type} (E$(safe_printf "%02d" "$original_ep"))${gl_end}"
                        if mv -- "$old_file" "./$new_name" 2>/dev/null; then
                            echo -e "  ${gl_lv}✓ 重命名为: $new_name${gl_bai}${gl_end}"
                            ((success_count++))
                        else
                            echo -e "  ${gl_hong}✗ 重命名失败${gl_bai}${gl_end}"
                            ((fail_count++))
                        fi
                    else
                        echo -e "${gl_huang}⚠ 文件不存在: $(basename "$old_file")${gl_bai}${gl_end}"
                        ((fail_count++))
                    fi
                    echo ""
                done
                ...
                echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}${gl_end}"
                echo -e "${gl_lv}成功:${gl_bai} $success_count 个文件${gl_end}"
                echo -e "${gl_hong}失败:${gl_bai} $fail_count 个文件${gl_end}"
                log_ok "重命名完成！"
                preview_mode=0
                auto_detected=0
                detection_results=()
