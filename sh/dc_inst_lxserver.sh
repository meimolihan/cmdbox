...
deploy_lxserver() {
    root_use || return 1
    clear
    echo -e "${gl_zi}>>> 落雪音乐 lxserver 一键部署${gl_bai}"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    ...
    DEFAULT_COMPOSE_DIR="/vol1/1000/compose/lxserver"
    ...
    mkdir -p "${COMPOSE_DIR}" || { log_error "目录创建失败"; exit_animation; return 1; }
    cd "${COMPOSE_DIR}" || { log_error "进入目录失败"; exit_animation; return 1; }
    ...
    DEFAULT_FRONT_PWD="yourfrontendpassword"
    read -r -e -p "${gl_bai}请设置 Web 管理密码（回车默认：${gl_huang}${DEFAULT_FRONT_PWD}${gl_bai}）(${gl_hong}0${gl_bai} 退出安装)：" FRONTEND_PASSWORD
    FRONTEND_PASSWORD=${FRONTEND_PASSWORD:-$DEFAULT_FRONT_PWD}

    if [ "$FRONTEND_PASSWORD" = "0" ]; then
        exit_script
        rm -rf "${COMPOSE_DIR}"
        return 1
    fi
    
    log_info "Web 管理密码已设置"
    echo -e ""

    read -r -e -p "${gl_bai}请设置 Web 播放器密码（回车默认：${gl_huang}同上${gl_bai}）(${gl_hong}0${gl_bai} 退出安装)：" WEBPLAYER_PASSWORD
    WEBPLAYER_PASSWORD=${WEBPLAYER_PASSWORD:-$FRONTEND_PASSWORD}

    if [ "$WEBPLAYER_PASSWORD" = "0" ]; then
        exit_script
        rm -rf "${COMPOSE_DIR}"
        return 1
    fi
    
    log_info "Web 播放器密码已设置"
    ...
    clean_old_container "lxserver" "lx-sync-server"
...
EOF
    ...
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    docker-ps-cn lxserver
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    LOCAL_IP=$(hostname -I | awk '{print $1}')
    log_info "部署完成！"
    log_info "访问地址：${gl_lv}http://${LOCAL_IP}:${HOST_PORT}${gl_bai}"
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    log_info "Web 管理密码：${gl_lv}${FRONTEND_PASSWORD}${gl_bai}"
    log_info "播放器密码：${gl_lv}${WEBPLAYER_PASSWORD}${gl_bai}"
