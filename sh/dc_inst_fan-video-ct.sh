...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="fan-video-ct 本地视频剪切工具 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/fan-video-ct"

# 默认访问端口（不传参时使用）
DEFAULT_PORT="8788"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="fan-video-ct"

# 媒体库挂载目录（只读挂载到容器 /media，供页面浏览与剪切读取；如需多目录挂多个卷）
DEFAULT_MEDIA_DIR="/vol2/1000/downloads/fan-video-ct"
# ====================================================================================
...
deploy_app() {
    ...
    # 检测硬件加速设备 /dev/dri（Intel/AMD 核显），存在则挂载（ffmpeg 可选硬件透传），否则跳过以免 compose 启动失败
    local DRI_DEVICES=""
    if [ -e "/dev/dri" ]; then
        log_info "检测到 /dev/dri 硬件加速设备，将挂载进容器"
        DRI_DEVICES="      devices:
         - /dev/dri:/dev/dri"
    else
        log_warn "未检测到 /dev/dri，跳过硬件加速挂载（软件转码兜底）"
    fi
...
services:
   fan-video-ct:
      image: mobufan/fan-video-ct:latest
      container_name: fan-video-ct
      network_mode: bridge
      restart: unless-stopped
      environment:
         - TZ=Asia/Shanghai
         # 文件浏览器默认根目录（指向媒体挂载点；如需换根目录请改本行或顶部 DEFAULT_MEDIA_DIR）
         - FCT_APP_MEDIA_DIR=/media
      volumes:
         - ./data:/data
         # 原视频目录只读挂载，供页面浏览与剪切读取（剪切输出默认保存在 data/output/）
         - ${DEFAULT_MEDIA_DIR}:/media:ro
      ports:
         - ${HOST_PORT}:8788
${DRI_DEVICES}
      deploy:
...
EOF
    ...
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    log_info "数据目录：${gl_huang}${COMPOSE_DIR}/data${gl_bai}（封面 covers/、剪切输出 output/、片头片尾预设 presets.json）"
    log_info "无账号密码（内网工具），请勿暴露到公网；如需修改媒体目录请编辑 docker-compose.yml"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
