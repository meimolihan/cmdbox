...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="Fan-Panel 管理面板 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/fan-panel"

# 默认访问端口（不传参时使用）
DEFAULT_PORT="3222"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="fan-panel"
# ====================================================================================
...
services:
   ${DEFAULT_CONTAINER_NAME}:
      image: mobufan/fan-panel:latest
...
EOF
    ...
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    log_info "配置目录：${gl_huang}${COMPOSE_DIR}/conf${gl_bai}"
    log_info "上传目录：${gl_huang}${COMPOSE_DIR}/uploads${gl_bai}"
    log_info "数据目录：${gl_huang}${COMPOSE_DIR}/database${gl_bai}"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
