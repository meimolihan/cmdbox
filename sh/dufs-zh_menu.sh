...
show_service_url() {
    ...
    if [ -z "$port" ];then
        ...
        port=$(echo "$exec_cmd" | grep -oE -- '--port [0-9]+' | grep -oE '[0-9]+' | head -1)
    fi
...
show_service_status() {
    ...
    if [ -z "$version" ] && [ -f /etc/dufs.conf ]; then
        version=$(grep -E '^VERSION=' /etc/dufs.conf 2>/dev/null | cut -d= -f2 | head -1)
    fi
