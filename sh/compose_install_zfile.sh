...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="ZFile 在线文件管理系统 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/zfile"

# 默认访问端口（不传参时使用），映射容器 8080
DEFAULT_PORT="8080"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="zfile"
# ====================================================================================
...
deploy_app() {
    ...
    if [ -d "${COMPOSE_DIR}" ] && [ "$(ls -A ${COMPOSE_DIR} 2>/dev/null)" ]; then
        log_warn "部署目录已存在，将使用现有目录"
    fi
    
    mkdir -p "${COMPOSE_DIR}/zfile" || { log_error "目录创建失败"; break_end; return 1; }
    mkdir -p "${COMPOSE_DIR}/file" || { log_error "文件目录创建失败"; break_end; return 1; }
...
EOF
    ...
    echo -e ""
    log_info "等待容器启动完成 ${gl_hong}.${gl_huang}.${gl_lv}.${gl_bai}"
    sleep 5
    ...
    echo ""
    echo -e "${gl_huang}>>> 部署完成！${gl_bai}"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    log_info "访问地址：${gl_lv}http://${LOCAL_IP}:${HOST_PORT}${gl_bai}"
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    log_info "文件存放目录：${gl_huang}${COMPOSE_DIR}/file${gl_bai}"
    log_info "数据库目录：${gl_huang}${COMPOSE_DIR}/zfile/db${gl_bai}"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    
    log_info "首次访问需要初始化系统，请按照向导设置管理员账户"
    log_info "ZFile 默认不支持 root 用户登录，请使用普通用户账户"
