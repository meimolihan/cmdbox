...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="fan-random 随机壁纸 API 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/fan-random"

# 默认访问端口（不传参时使用）
DEFAULT_PORT="8588"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="fan-random"

# pc 桌面端壁纸目录（In-Container 路径固定为 /fan-random/public/pc）
DEFAULT_PC_IMAGE_DIR="public/pc"

# mp 移动端壁纸目录（In-Container 路径固定为 /fan-random/public/mp）
DEFAULT_MP_IMAGE_DIR="public/mp"

# 原始图片目录（classify 脚本输入，In-Container 路径固定为 /fan-random/photos）
...
deploy_app() {
    ...
    # 获取本机 IP，用于容器 PUBLIC_HOST 与部署完成后的访问地址
    local HOST_IP
    HOST_IP=$(hostname -I 2>/dev/null | awk '{print $1}')
    [ -z "$HOST_IP" ] && HOST_IP="127.0.0.1"

    # 创建壁纸目录与原始图片目录（挂载到容器的宿主机路径，可直接管理图片，无需重建镜像）
    mkdir -p "${DEFAULT_PC_IMAGE_DIR}" "${DEFAULT_MP_IMAGE_DIR}" "${DEFAULT_PHOTOS_DIR}"
    log_ok "壁纸目录已就绪：${gl_huang}${COMPOSE_DIR}/${DEFAULT_PC_IMAGE_DIR}${gl_bai}（pc 桌面端）/ ${gl_huang}${COMPOSE_DIR}/${DEFAULT_MP_IMAGE_DIR}${gl_bai}（mp 移动端）"
    log_ok "原始图片目录：${gl_huang}${COMPOSE_DIR}/${DEFAULT_PHOTOS_DIR}${gl_bai}"
...
services:
   fan-random:
      image: mobufan/fan-random:latest
      container_name: fan-random
      network_mode: bridge
      restart: always
      environment:
         - TZ=Asia/Shanghai
         - PORT=3000
         # 对外访问地址（可选，用于启动日志显示；留空则显示 http://0.0.0.0:端口）
         - PUBLIC_HOST=http://${HOST_IP}:${HOST_PORT}
      volumes:
         # pc 桌面端（横屏）壁纸目录，宿主机直接放图即生效（无需重启/重建镜像）
         - ./public/pc:/fan-random/public/pc
         # mp 移动端（竖屏）壁纸目录
         - ./public/mp:/fan-random/public/mp
         # 原始图片目录（可用于 classify.py / classify.sh 批量处理）
         - ./photos:/fan-random/photos
      ports:
         - ${HOST_PORT}:3000
      deploy:
         resources:
...
EOF
    ...
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    log_info "部署完成！"
    log_info "访问地址：${gl_lv}http://${HOST_IP}:${HOST_PORT}/${gl_bai}（自适应，自动识别手机/电脑）"
    log_info "pc 桌面端：${gl_lv}http://${HOST_IP}:${HOST_PORT}/pc${gl_bai}  |  mp 移动端：${gl_lv}http://${HOST_IP}:${HOST_PORT}/mp${gl_bai}"
    log_info "JSON 查询：${gl_huang}http://${HOST_IP}:${HOST_PORT}/pc?type=json${gl_bai}"
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    log_info "壁纸目录：${gl_huang}${COMPOSE_DIR}/public/pc${gl_bai}（pc 桌面端）/ ${gl_huang}${COMPOSE_DIR}/public/mp${gl_bai}（mp 移动端），放入新图片后自动生效"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
