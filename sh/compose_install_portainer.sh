...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="Portainer 容器管理面板（中文版） 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/portainer"

# 默认 Web 访问端口（不传参时使用），映射容器 9000
DEFAULT_PORT="9000"

# Agent 通信端口（固定端口，通常无需修改）
AGENT_PORT="8000"
...
deploy_app() {
    ...
    mkdir -p "${COMPOSE_DIR}/dockerconfig/portainer" || { log_error "目录创建失败"; break_end; return 1; }
    ...
    log_info "Agent 端口：${gl_lv}${AGENT_PORT}${gl_bai}"
    ...
    check_and_open_port "${AGENT_PORT}"
...
EOF
    ...
    log_info "Agent 地址：${gl_lv}${LOCAL_IP}:${AGENT_PORT}${gl_bai}"
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
