          ...
          if [ "$need" = 1 ]; then
            ...
            if [ "$THUMB" = "webp" ] || [ "$THUMB" = "both" ]; then
              ffmpeg -hide_banner -loglevel error -ss "$ss" -i "$thumb_src" -frames:v 1 \
                ...
                || { thumb_fail=$((thumb_fail+1)); echo "    ✗ webp 失败: $(basename "$out" .mp4)"; }
            fi
            if [ "$THUMB" = "jpg" ] || [ "$THUMB" = "both" ]; then
              ffmpeg -hide_banner -loglevel error -ss "$ss" -i "$thumb_src" -frames:v 1 \
                ...
                || { thumb_fail=$((thumb_fail+1)); echo "    ✗ jpg 失败: $(basename "$out" .mp4)"; }
            fi
          else
            thumb_skip=$((thumb_skip+1))
            case "$THUMB" in
              webp) echo "    - $(basename "$base").thumb.webp 已存在" ;;
              jpg)  echo "    - $(basename "$base").thumb.jpg 已存在" ;;
              both) echo "    - $(basename "$base").thumb.webp/.jpg 已存在" ;;
            esac
          fi
        fi
...
main() {
  ...
  echo "===== 全局配置：MODE=$MODE MAXDEPTH=$MAXDEPTH  mindepth固定=1 ====="
  echo "停止标记文件：$STOP_FILE"
