...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="Chromium 网页版浏览器 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/chrome"

# 默认访问端口（不传参时使用），映射容器 3000（HTTP）
DEFAULT_PORT="3000"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="chrome"
# ====================================================================================
...
deploy_app() {
    ...
    mkdir -p "${COMPOSE_DIR}/chromium-config" || { log_error "目录创建失败"; break_end; return 1; }
    cd "${COMPOSE_DIR}" || { log_error "进入目录失败"; break_end; return 1; }

    if [ -z "${HOST_PORT}" ]; then
        read -r -e -p "${gl_bai}请输入映射端口（HTTP，回车默认：${gl_huang}${DEFAULT_PORT}${gl_bai}，HTTPS 默认为端口+1）(${gl_hong}0${gl_bai} 退出安装)：" input_port
    ...
    local HTTPS_PORT=$((HOST_PORT + 1))
    log_info "HTTP 端口：${gl_lv}${HOST_PORT}${gl_bai}"
    log_info "HTTPS 端口：${gl_lv}${HTTPS_PORT}${gl_bai}"
    ...
    if ! check_port_available $HTTPS_PORT; then
        ...
        NEW_PORT=$(get_free_port $((HTTPS_PORT + 1)))
        if [ -n "$NEW_PORT" ]; then
            log_info "自动分配新 HTTPS 端口：${gl_lv}${NEW_PORT}${gl_bai}"
            HTTPS_PORT=$NEW_PORT
        else
            log_error "无法找到可用端口，请手动指定"
            break_end
...
services:
   ${DEFAULT_CONTAINER_NAME}:
      ...
      environment:
         ...
         - LANG=zh_CN.UTF-8
         - LC_ALL=zh_CN.UTF-8
         - CHROME_LANGUAGE=zh-CN
         # - HTTP_PROXY=http://10.10.10.252:7893   # 代理
...
EOF
    ...
    log_info "HTTPS 访问地址：${gl_lv}https://${LOCAL_IP}:${HTTPS_PORT}${gl_bai}"
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
