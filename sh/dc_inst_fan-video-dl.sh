...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="fan-video-dl 视频下载器 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/fan-video-dl"

# 默认访问端口（不传参时使用）
DEFAULT_PORT="5200"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="fan-video-dl"
# ====================================================================================
...
services:
   fan-video-dl:
      image: mobufan/fan-video-dl:latest
      container_name: fan-video-dl
      network_mode: bridge
...
EOF
    ...
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    log_info "数据目录：${gl_huang}${COMPOSE_DIR}/data${gl_bai} (配置/任务) / ${gl_huang}${COMPOSE_DIR}/downloads${gl_bai} (下载文件)"
    log_info "默认账号：${gl_lv}admin${gl_bai}，默认密码 ${gl_lv}admin888${gl_bai}，登录后请及时在「账户」中修改"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
