...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="MinDoc 文档管理系统 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/mindoc"

# 默认访问端口（不传参时使用），映射容器 8181
DEFAULT_PORT="8181"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="mindoc"
# ====================================================================================
...
deploy_app() {
    ...
    mkdir -p "${COMPOSE_DIR}/mindoc" || { log_error "目录创建失败"; break_end; return 1; }
...
services:
   ${DEFAULT_CONTAINER_NAME}:
      ...
      ports:
         - ${HOST_PORT}:8181
      volumes:
         - ./${DEFAULT_CONTAINER_NAME}/conf://mindoc/conf
         - ./${DEFAULT_CONTAINER_NAME}/static://mindoc/static
         - ./${DEFAULT_CONTAINER_NAME}/views://mindoc/views
         - ./${DEFAULT_CONTAINER_NAME}/uploads://mindoc/uploads
         - ./${DEFAULT_CONTAINER_NAME}/runtime://mindoc/runtime
         - ./${DEFAULT_CONTAINER_NAME}/database://mindoc/database
      environment:
...
EOF
    ...
    log_info "默认登录：${gl_huang}admin / 123456${gl_bai}"
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
