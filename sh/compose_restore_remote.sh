...
interactive_setup() {
    ...
    ask_if_empty BASE_URL      "请输入服务器地址"        "http://10.10.10.251:5000/myfile/compose/downloads"
    ask_if_empty TARGET_ROOT   "请输入下载解压根目录"    "/vol1/1000/compose"


    echo ""
    echo -e "${gl_lv}配置完成:${gl_bai}"
    echo -e "压缩包目录 : ${gl_huang}$ARCHIVE_DIR${gl_bai}"
    echo -e "服务器地址 : ${gl_huang}$BASE_URL${gl_bai}"
...
download_and_extract() {
    ...
    if ! gzip -t "$tmp_file" 2>/dev/null; then
        log_error "下载的文件不是有效的 gzip 格式: $filename"
        rm -f "$tmp_file"
        return 1
    fi
...
main_menu() {
    ...
    while true; do
        clear
        echo -e "${gl_zi}>>> Compose 项目远程还原工具${gl_bai}"
        echo -e "${gl_bufan}——————————————————————————————————————————————————————————${gl_bai}"
        echo -e "${gl_bai}压缩包目录 : ${gl_huang}$ARCHIVE_DIR${gl_bai}"
        echo -e "${gl_bai}服务器地址 : ${gl_huang}$BASE_URL${gl_bai}"
        ...
        88)
            ...
            echo -e "${gl_bufan}——————————————————————————————————————————————————————————${gl_bai}"
            read -r -e -p "$(echo -e "${gl_bai}输入URL(0返回): ")" url
