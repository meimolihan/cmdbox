...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="Litepan 文件管理面板 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/litepan"

# 默认访问端口（不传参时使用）
DEFAULT_PORT="5211"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="litepan"
# ====================================================================================
...
services:
   ${DEFAULT_CONTAINER_NAME}:
      ...
      ports:
         - ${HOST_PORT}:5211
         # 如果启用了 Emby 反代，还需要额外增加对应端口映射，例如 -p 8098:8098。
      environment:
         TZ: Asia/Shanghai
      volumes:
         - ./data:/app/data
