...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="Dufs-ZH 中文版私有网盘 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/dufs-zh"

# 默认访问端口（不传参时使用）
DEFAULT_PORT="5000"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="dufs-zh"

# 默认启动命令（可自定义）
# /data           - 匿名可读，无上传编辑权限
# /data -A        - 允许所有操作（上传、编辑、删除）
# /data -A --compress none - 全开 + 禁用压缩（大文件更高效）
# /data -a admin:password@/:rw - 设置认证访问
DEFAULT_COMMAND="/data -A"
# ====================================================================================
...
EOF
    ...
    log_info "启动命令：${gl_lv}${DEFAULT_COMMAND}${gl_bai}"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
